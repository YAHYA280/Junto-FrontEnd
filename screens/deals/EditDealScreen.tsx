...
import { UpdateHotDealInput, HotDeal } from '../../shared/types/deal';

export const EditDealScreen: React.FC = () => {
  ...
  const loadDeal = async () => {
    ...
    try {
      ...
      setPriceOriginal(dealData.priceOriginal || '');
      setPriceDeal(dealData.priceDeal || '');
      setCurrency(dealData.currency || 'MAD');
      setQuantityTotal(dealData.quantityTotal?.toString() || '');
      setMerchantName(dealData.hot?.merchantName || '');
      setRedemptionNotes(dealData.hot?.redemptionNotes || '');
    } catch (err: any) {
      console.error('[EditDeal] Error loading deal:', err);
      Alert.alert('Error', err.message || 'Failed to load deal');
      router.back();
    } finally {
      setIsLoading(false);
    }
  ...
  const handleSubmit = async () => {
    ...
    try {
      const dealData: UpdateHotDealInput = {
        title: title.trim(),
      ...
      await dealsApi.updateHotDeal(id as string, dealData);
  ...
  return (
    ...
    >
      ...
      <View style={[styles.header, { backgroundColor: colors.surface }]}>
        ...
        <Text style={[styles.headerTitle, { color: colors.text }]}>Edit Deal</Text>
      ...
      >
        ...
        <AnimatedView animation="scale" delay={400} duration={500}>
          <Button
            title={isSubmitting ? 'Updating Deal...' : 'Update Deal'}
